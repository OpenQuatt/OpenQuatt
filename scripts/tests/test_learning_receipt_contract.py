...
class LearningReceiptContractTest(unittest.TestCase):
    ...
    def test_controller_flow_invalid_input_cannot_bypass_receipt(self) -> None:
        ...
        self.assertIn("lph = oq_flow_meter::linear_lph(", block)
        self.assertIn("const bool valid = isfinite(lph);", block)
        valid = block.index("const bool valid = isfinite(lph);")
        observe = block.index(
            "oq_sources::controller_flow.observe(accepted_lph, oq_sources::monotonic_ms(), valid);"
        )
        returned = block.index("return accepted_lph;", observe)
        self.assertLess(valid, observe)
        self.assertLess(observe, returned)
        self.assertIn("const float accepted_lph = valid ?", block)
