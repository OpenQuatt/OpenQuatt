...
De feature is uitsluitend bestemd voor `heatpump_controller_q`, Single en Duo. Alleen het Q-profiel zet `OQ_POWER_HOUSE_LEARNING_TARGET=1`. De learningheaders houden de leerkern buiten overige firmwaretargets; de flag en het profielpakket maken collector en webcapability uitsluitend op Q beschikbaar. Geen learnerpad schrijft regelparameters.

## Herintegratie op actuele dev (20 september 2026)

De herintegratie behoudt de huidige regelarchitectuur: de centrale Home Assistant-ingressheartbeat uit #700 blijft de freshnessbron voor live HA-inputs; het V2-prestatie- en Power Input-pad uit #675 blijft ongewijzigd; de Power House room-recovery/failover-wijzigingen uit #666 en #714 blijven actief. De instelbare controller-flowmeter uit #691 hoort nu bij de fysieke learning-context: wisselen tussen Huba/Custom of wijzigen van pulsen per liter onderbreekt een lopend interval zonder opgeslagen historie te wissen.

Legacy controllerplatforms zijn sinds #681 verwijderd. Passief leren wordt daarom alleen voor Q Single/Duo gebouwd.
...
| 2. Bronmetadata en firmwarecollector | Werkelijke source-route/timestamps, Q-only allowlist, opt-in, diagnostiek | Alle ondersteunde entry paths gecontroleerd; ontbrekend bewijs faalt gesloten; niet-Q firmware bevat geen learner |
...
De hardwarepoort omvat Q Single en Q Duo. Valideer beide firmwaretargets en voer representatieve runtimebelasting uit op de ondersteunde Q-connectiviteit. Meet interne heap, grootste vrije blok, minimum-sinds-boot, stack-watermarks en timing onder gecombineerde belasting. Desktoptests of succesvolle configuratievalidatie vervangen deze metingen niet. Het 16 KiB learnerjournal ligt direct na het crasharchief binnen `openquatt_data`; bestaande archiefoffsets blijven behouden.
