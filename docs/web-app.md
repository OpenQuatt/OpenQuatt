...
De `Boiler power test` is een diagnostische servicetest, geen vereiste vermogenskalibratie. De test controleert of de ketel op commando start, de flow stabiel blijft en warmte veilig aan het systeem wordt overgedragen. Het gemeten waterzijdige vermogen is een indicatie die mede afhangt van flow, sensoren en testtemperaturen. Voor de normale regeling volstaat het ingestelde `Boiler rated heat power`; sommige OpenTherm-ketels leveren daarnaast hun nominale maximum via ID15, waarmee de test een passend en veilig operating point kan kiezen. Een normaal afgeronde, niet-begrensde meting die de kwaliteitscriteria haalt kan als voorstel voor `Boiler rated heat power` worden toegepast; een empirisch OpenTherm-resultaat zonder ID15 vereist daarvoor een tweede bevestiging binnen 30 seconden. Bij een aan/uit-ketel blijft de fysieke aansturing binair en verandert een vermogensmeting dat regelgedrag niet.
