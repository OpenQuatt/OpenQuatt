...
| `Gebruiksstatistieken` | Wel of niet beperkte technische systeemstatus, feature-instellingen en crashinformatie delen | Tijdens een nieuwe Quick Start staat delen standaard aan en kan het hier worden uitgezet. |
...
Tijdens een nieuwe Quick Start staat delen standaard aan en verschijnt de opt-out vóór het afronden. De keuze wordt pas opgeslagen wanneer die stap werkelijk wordt geopend. Je kunt de keuze later wijzigen via **Instellingen → Systeem → Gebruiksstatistieken**. Zolang Quick Start niet is afgerond, wordt niets verzonden. Daarna, of wanneer je delen later zelf aanzet, verstuurt OpenQuatt vrijwel direct en vervolgens ongeveer elk uur één klein statusbericht naar de centrale OpenQuatt-loggingserver. Na een echte firmwarecrash kan daarnaast één beperkt crashbericht worden verstuurd.
...
Een crashbericht bevat geen reguliere logregels. Het bevat uitsluitend de crashreden en ruwe oorzaakcode, processorcore, program counter, een foutadres wanneer dat voor de exception betekenisvol is, en maximaal zestien ruwe backtrace-adressen per core. Tijdstip en uptime komen waar mogelijk uit de laatste breadcrumb vóór de crash. De huidige rapporterende build heeft een eigen volledige ELF-SHA256. Voor de build die de crash vastlegde bevat het bericht daarnaast de GitHub-repository, Git-commit, deterministische build-epoch en het firmwaretarget. Ruwe adressen mogen alleen worden gesymboliseerd wanneer een gereconstrueerd ELF exact dezelfde SHA256 heeft als deze captured build; bij een afwijking blijven ze ongesymboliseerd.

Wanneer delen voor het eerst actief wordt, maakt de controller met de hardware-randomgenerator een UUIDv4 aan en bewaart die lokaal. Een UUIDv4 heeft 122 willekeurige bits; zelfs bij één miljoen installaties is de kans op minstens één dubbel ID kleiner dan ongeveer `10^-25`. Dit ID blijft gelijk na een OTA-update en wanneer je delen tijdelijk uitzet. Je kunt het bekijken via **Instellingen → Systeem → Gebruiksstatistieken**. Een fabrieksreset maakt een nieuw ID. De keuze en het ID worden niet via een instellingenbackup naar een andere controller gekopieerd. Uitzetten sluit de datagate direct. Als er mogelijk retained crashdata op de centrale broker staat, bewaart de controller alleen de cleanup-intentie, het bestaande installatie-ID en zo nodig het lokaal wachtende crashbewijs om dat topic met een lege retained payload te wissen; pas na de QoS 1-bevestiging wordt het lokale crashbericht verwijderd. Dit cleanup-pad mag nooit nieuwe telemetry versturen. Na een mislukte statusverzending maakt iedere retry een verse momentopname, maar behoudt binnen dezelfde retryreeks het `message_id`. Een crashsnapshot behoudt zijn eigen `message_id` totdat de broker de QoS 1-publicatie heeft bevestigd, zodat verloren bevestigingen en reboots veilig kunnen worden gededupliceerd.

De statistiekenclient staat los van de configureerbare [MQTT inputbronnen](mqtt.md): hij publiceert alleen status- en crashberichten, subscribed nergens op en schakelt ESPHome MQTT-discovery, entiteitspublicaties en reguliere logexport niet in. Het statusbericht blijft met QoS 1 en zonder retain gepubliceerd op `openquatt/devices/<installation-id>/telemetry`. Het crashbericht gebruikt QoS 1 met retain op `openquatt/devices/<installation-id>/crash` en representeert bewust alleen de laatst bekende crash van de installatie. Een volgende crash vervangt deze retained waarde. Een build zonder geconfigureerde centrale loggingserver maakt ook wanneer delen aanstaat geen externe verbinding.
