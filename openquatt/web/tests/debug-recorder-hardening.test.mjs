...
test("packed rows vergroten de retentie van de volledige dev-debugset", () => {
  ...
  assert.equal(rowBytes, 748);
  assert.equal(capacity, 1401);
  ...
  // compacte text-state toe (+2 B/rij). Issue #746 voegt tien flow- en
  // pompdiagnostiekkolommen toe (4x select, 1x number, 5x sensor = +32 B/rij).
  // De retentie blijft met ~3,9 uur ruim boven de maximaal instelbare
  // opnameduur van 1 uur.
