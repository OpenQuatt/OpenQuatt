...
struct ModelValidationResult {
  ...
  // Agreement of heat-loss estimates only; it does not validate capacity C.
...
inline ModelValidationResult validate_house_models(const AdviceResult& batch, const ThermalModelState& thermal,
  ...
  // Diagnostic only: C is not independently validated. Quantized room readings
  // can produce a plausible but biased C even when numerical readiness passes.
  // Do not use this estimate to veto a batch that passed its own quality checks.
