...
CONF_ON_WRITE_STARTED = "on_write_started"
CONF_ON_WRITE_APPLIED = "on_write_applied"
CONF_WEB_AUTH = "web_auth"

runtime_ns = cg.esphome_ns.namespace("openquatt_odu_runtime_frequency")
OpenQuattOduRuntimeFrequency = runtime_ns.class_(
    "OpenQuattOduRuntimeFrequency", cg.Component
)
RuntimeWriteStartedTrigger = runtime_ns.class_(
    "RuntimeWriteStartedTrigger", automation.Trigger.template()
)
RuntimeWriteAppliedTrigger = runtime_ns.class_(
    "RuntimeWriteAppliedTrigger", automation.Trigger.template()
)
...
CONFIG_SCHEMA = cv.Schema(
    {
        cv.GenerateID(): cv.declare_id(OpenQuattOduRuntimeFrequency),
        ...
        cv.Optional(CONF_ON_WRITE_STARTED): automation.validate_automation(
            {
                cv.GenerateID(CONF_TRIGGER_ID): cv.declare_id(
                    RuntimeWriteStartedTrigger
                ),
            }
        ),
        cv.Optional(CONF_ON_WRITE_APPLIED): automation.validate_automation(
            {
                cv.GenerateID(CONF_TRIGGER_ID): cv.declare_id(
                    RuntimeWriteAppliedTrigger
                ),
            }
        ),
    }
...
async def to_code(config):
    ...
    cg.add_global(runtime_ns.using)
    ...
    for conf in config.get(CONF_ON_WRITE_STARTED, []):
        trigger = cg.new_Pvariable(conf[CONF_TRIGGER_ID], var)
        await automation.build_automation(trigger, [], conf)

    for conf in config.get(CONF_ON_WRITE_APPLIED, []):
        trigger = cg.new_Pvariable(conf[CONF_TRIGGER_ID], var)
        await automation.build_automation(trigger, [], conf)
