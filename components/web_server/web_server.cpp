...
static UrlMatch match_url(const char* url_ptr, size_t url_len, bool only_domain, bool is_post = false) {
  ...
  if (url_len < 2 || url_ptr[0] != '/') return UrlMatch{};
  ...
  if (!s2) return UrlMatch{};

  UrlMatch match{};
  match.domain = make_ref(s1, s2);
  match.valid = true;

  if (only_domain || s2 >= end) return match;
  ...
  if (seg2.empty() || (s3 && seg3.empty()) || (s4 && seg4.empty())) return UrlMatch{};
...
void DeferredUpdateEventSource::deferrable_send_state(void* source, const char* event_type,
  ...
  if (!entities_iterator_.completed() && 0 != strcmp(event_type, "state_detail_all")) return;

  if (source == nullptr) return;
  if (event_type == nullptr) return;
  if (message_generator == nullptr) return;

  if (0 != strcmp(event_type, "state_detail_all") && 0 != strcmp(event_type, "state")) {
    ESP_LOGE(TAG, "Can't defer non-state event");
  }

  if (!deferred_queue_.empty()) process_deferred_queue_();
...
void WebServer::loop() {
  ...
  if (!this->events_.loop()) this->disable_loop();
}
...
bool WebServer::is_request_origin_allowed_(AsyncWebServerRequest* request, const std::string& origin) {
  ...
  if (scheme_sep != std::string::npos) {
    ...
    if (!host.empty() && origin.compare(scheme_sep + 3, std::string::npos, host) == 0) return true;
  }
  ...
  for (const char* allowed_origin : this->allowed_origins_) {
    ...
    if (allowed_origin[0] == '*' && allowed_origin[1] == '\0') return true;
...
static void set_json_id(JsonObject& root, EntityBase* obj, const char* prefix, JsonDetail start_config) {
  ...
  *p++ = '/';
  ...
  if (start_config == DETAIL_ALL) {
    ...
    if (is_disabled) root[ESPHOME_F("is_disabled_by_default")] = is_disabled;
  }
}
...
void WebServer::handle_sensor_request(AsyncWebServerRequest* request, const UrlMatch& match) {
  for (sensor::Sensor* obj : App.get_sensors()) {
    ...
    if (!entity_match.matched) continue;
...
json::SerializationBuffer<> WebServer::sensor_json_(sensor::Sensor* obj, float value, JsonDetail start_config) {
  ...
  if (start_config == DETAIL_ALL) {
    ...
    if (!uom_ref.empty()) root[ESPHOME_F("uom")] = uom_ref.c_str();
  }
...
json::SerializationBuffer<> WebServer::text_sensor_state_json_generator(WebServer* web_server, void* source) {
  return web_server->text_sensor_json_((text_sensor::TextSensor*)(source), ((text_sensor::TextSensor*)(source))->state,
                                       DETAIL_STATE);
}
...
json::SerializationBuffer<> WebServer::fan_json_(fan::Fan* obj, JsonDetail start_config) {
  ...
  if (obj->get_traits().supports_oscillation()) root[ESPHOME_F("oscillation")] = obj->oscillating;
  if (start_config == DETAIL_ALL) {
    this->add_sorting_info_(root, obj);
  }
...
json::SerializationBuffer<> WebServer::cover_json_(cover::Cover* obj, JsonDetail start_config) {
  ...
  if (obj->get_traits().get_supports_position()) root[ESPHOME_F("position")] = obj->position;
  if (obj->get_traits().get_supports_tilt()) root[ESPHOME_F("tilt")] = obj->tilt;
  if (start_config == DETAIL_ALL) {
...
void WebServer::handle_climate_request(AsyncWebServerRequest* request, const UrlMatch& match) {
  for (auto* obj : App.get_climates()) {
    ...
    parse_cstr_param_(
        request, ESPHOME_F("preset"), call,
        static_cast<climate::ClimateCall& (climate::ClimateCall::*)(const char*, size_t)>(&decltype(call)::set_preset));
...
json::SerializationBuffer<> WebServer::climate_json_(climate::Climate* obj, JsonDetail start_config) {
  ...
  if (start_config == DETAIL_ALL) {
    ...
    if (!traits.get_supported_custom_fan_modes().empty()) {
      ...
      for (auto const& custom_fan_mode : traits.get_supported_custom_fan_modes()) opt.add(custom_fan_mode);
    }
    ...
    if (!traits.get_supported_custom_presets().empty()) {
      ...
      for (auto const& custom_preset : traits.get_supported_custom_presets()) opt.add(custom_preset);
    }
  ...
  if (traits.has_feature_flags(climate::CLIMATE_SUPPORTS_TWO_POINT_TARGET_TEMPERATURE |
    ...
    if (!has_state) root[ESPHOME_F("state")] = root[ESPHOME_F("target_temperature")];
  }
...
json::SerializationBuffer<> WebServer::water_heater_json_(water_heater::WaterHeater* obj, JsonDetail start_config) {
  ...
  if (start_config == DETAIL_ALL) {
    ...
    for (auto m : traits.get_supported_modes()) modes.add(json_state_str(water_heater::water_heater_mode_to_string(m)));
  ...
  if (traits.get_supports_current_temperature()) {
    ...
    if (!std::isnan(current)) root[ESPHOME_F("current_temperature")] = current;
  }

  if (traits.get_supports_two_point_target_temperature()) {
    float low = obj->get_target_temperature_low();
    float high = obj->get_target_temperature_high();
    if (!std::isnan(low)) root[ESPHOME_F("target_temperature_low")] = low;
    if (!std::isnan(high)) root[ESPHOME_F("target_temperature_high")] = high;
  } else {
    float target = obj->get_target_temperature();
    if (!std::isnan(target)) root[ESPHOME_F("target_temperature")] = target;
  }
...
bool WebServer::canHandle(AsyncWebServerRequest* request) const {
...
#if !defined(USE_ESP32) && defined(USE_ARDUINO)
  if (url == ESPHOME_F("/events")) return true;
#endif
#ifdef USE_WEBSERVER_CSS_INCLUDE
  if (url == ESPHOME_F("/0.css")) return true;
#endif
#ifdef USE_WEBSERVER_JS_INCLUDE
  if (url == ESPHOME_F("/0.js")) return true;
#endif
  ...
  if (!is_get_or_post) return false;

  // Check GET-only domains - use ESPHOME_F to keep strings in flash on ESP8266
  if (is_get) {
#ifdef USE_SENSOR
    if (match.domain_equals(ESPHOME_F("sensor"))) return true;
#endif
#ifdef USE_BINARY_SENSOR
    if (match.domain_equals(ESPHOME_F("binary_sensor"))) return true;
#endif
#ifdef USE_TEXT_SENSOR
    if (match.domain_equals(ESPHOME_F("text_sensor"))) return true;
#endif
#ifdef USE_EVENT
    if (match.domain_equals(ESPHOME_F("event"))) return true;
#endif
  }
  ...
  if (is_get_or_post) {
#ifdef USE_SWITCH
    if (match.domain_equals(ESPHOME_F("switch"))) return true;
#endif
#ifdef USE_BUTTON
    if (match.domain_equals(ESPHOME_F("button"))) return true;
#endif
#ifdef USE_FAN
    if (match.domain_equals(ESPHOME_F("fan"))) return true;
#endif
#ifdef USE_LIGHT
    if (match.domain_equals(ESPHOME_F("light"))) return true;
#endif
#ifdef USE_COVER
    if (match.domain_equals(ESPHOME_F("cover"))) return true;
#endif
#ifdef USE_NUMBER
    if (match.domain_equals(ESPHOME_F("number"))) return true;
#endif
#ifdef USE_DATETIME_DATE
    if (match.domain_equals(ESPHOME_F("date"))) return true;
#endif
#ifdef USE_DATETIME_TIME
    if (match.domain_equals(ESPHOME_F("time"))) return true;
#endif
#ifdef USE_DATETIME_DATETIME
    if (match.domain_equals(ESPHOME_F("datetime"))) return true;
#endif
#ifdef USE_TEXT
    if (match.domain_equals(ESPHOME_F("text"))) return true;
#endif
#ifdef USE_SELECT
    if (match.domain_equals(ESPHOME_F("select"))) return true;
#endif
#ifdef USE_CLIMATE
    if (match.domain_equals(ESPHOME_F("climate"))) return true;
#endif
#ifdef USE_LOCK
    if (match.domain_equals(ESPHOME_F("lock"))) return true;
#endif
#ifdef USE_VALVE
    if (match.domain_equals(ESPHOME_F("valve"))) return true;
#endif
#ifdef USE_ALARM_CONTROL_PANEL
    if (match.domain_equals(ESPHOME_F("alarm_control_panel"))) return true;
#endif
#ifdef USE_UPDATE
    if (match.domain_equals(ESPHOME_F("update"))) return true;
#endif
#ifdef USE_WATER_HEATER
    if (match.domain_equals(ESPHOME_F("water_heater"))) return true;
#endif
#ifdef USE_INFRARED
    if (match.domain_equals(ESPHOME_F("infrared"))) return true;
#endif
#ifdef USE_RADIO_FREQUENCY
    if (match.domain_equals(ESPHOME_F("radio_frequency"))) return true;
#endif
  }
