...
static const LogString* phy_mode_to_log_string(WiFi8266PhyMode mode) {
  if (mode == WIFI_8266_PHY_MODE_11B) return LOG_STR("11B");
  if (mode == WIFI_8266_PHY_MODE_11G) return LOG_STR("11G");
  if (mode == WIFI_8266_PHY_MODE_11N) return LOG_STR("11N");
  return LOG_STR("Auto");
}
#endif
// Use if-chain instead of switch to avoid jump table in RODATA (wastes RAM on ESP8266)
static const LogString* retry_phase_to_log_string(WiFiRetryPhase phase) {
  if (phase == WiFiRetryPhase::INITIAL_CONNECT) return LOG_STR("INITIAL_CONNECT");
#ifdef USE_WIFI_FAST_CONNECT
  if (phase == WiFiRetryPhase::FAST_CONNECT_CYCLING_APS) return LOG_STR("FAST_CONNECT_CYCLING");
#endif
  if (phase == WiFiRetryPhase::EXPLICIT_HIDDEN) return LOG_STR("EXPLICIT_HIDDEN");
  if (phase == WiFiRetryPhase::SCAN_CONNECTING) return LOG_STR("SCAN_CONNECTING");
  if (phase == WiFiRetryPhase::RETRY_HIDDEN) return LOG_STR("RETRY_HIDDEN");
  if (phase == WiFiRetryPhase::RESTARTING_ADAPTER) return LOG_STR("RESTARTING");
...
void WiFiComponent::init_preferences_() {
  if (this->preferences_initialized_) return;
  this->preferences_initialized_ = true;
  // Bind once, before runtime STA is loaded. disable/enable must not change the
  // storage key merely because has_sta() now contains provisioned credentials.
  uint32_t hash = this->has_sta() ? App.get_config_version_hash() : 88491487UL;

  this->pref_ = global_preferences->make_preference<wifi::SavedWifiSettings>(hash, true);
#if !defined(USE_WIFI_FAST_CONNECT) || defined(USE_WIFI_FAST_CONNECT_IN_FLASH)
  ...
  this->provisioning_required_ = !this->has_sta() && (!this->pref_.load(&saved) || saved.ssid[0] == '\0');
}
...
bool WiFiComponent::clear_saved_sta_checked() {
  ...
  if (!this->fast_connect_pref_.save(&empty_fast)) return false;
  if (!global_preferences->sync()) return false;
  SavedWifiSettings verified{};
  if (!this->pref_.load(&verified) || memcmp(&verified, &empty, sizeof(empty)) != 0) return false;
  SavedWifiFastConnectSettings verified_fast{};
  if (!this->fast_connect_pref_.load(&verified_fast) || memcmp(&verified_fast, &empty_fast, sizeof(empty_fast)) != 0)
    return false;
  return true;
}
...
void WiFiComponent::start() {
  ...
  if (!this->has_sta() && esp32_improv::global_improv_component != nullptr) {
    if (this->wifi_mode_(true, {})) esp32_improv::global_improv_component->start();
  }
#endif
...
network::IPAddresses WiFiComponent::get_ip_addresses() {
  if (this->has_sta()) return this->wifi_sta_ip_addresses();

#ifdef USE_WIFI_AP
  if (this->has_ap()) return {this->wifi_soft_ap_ip()};
#endif  // USE_WIFI_AP

  return {};
}
network::IPAddress WiFiComponent::get_dns_address(int num) {
  if (this->has_sta()) return this->wifi_dns_ip_(num);
  return {};
}
...
void WiFiComponent::persist_pending_credentials_() {
  ...
  SavedWifiSettings candidate{};
  strncpy(candidate.ssid, this->sta_[0].get_ssid().c_str(), sizeof(candidate.ssid) - 1);
  strncpy(candidate.password, this->sta_[0].get_password().c_str(), sizeof(candidate.password) - 1);
  SavedWifiSettings verified{};
...
[[nodiscard]] inline static bool wifi_scan_result_is_better(const WiFiScanResult& a, const WiFiScanResult& b) {
  ...
  if (a.get_matches() && !b.get_matches()) return true;
  if (!a.get_matches() && b.get_matches()) return false;
...
bool WiFiScanResult::matches(const WiFiAP& config) const {
  if (config.get_hidden()) {
    ...
    // don't match SSID
    if (!this->is_hidden_) return false;
  } else if (!config.ssid_.empty()) {
    // check if SSID matches
    if (this->ssid_ != config.ssid_) return false;
  } else {
    // network is configured without SSID - match other settings
  }
  // If BSSID configured, only match for correct BSSIDs
  if (config.has_bssid() && config.get_bssid() != this->bssid_) return false;

#ifdef USE_WIFI_WPA2_EAP
  // BSSID requires auth but no PSK or EAP credentials given
  if (this->with_auth_ && (config.password_.empty() && !config.get_eap().has_value())) return false;
  ...
  if (config.password_.empty() == this->with_auth_) return false;
#endif
...
void WiFiComponent::notify_connect_state_listeners_() {
  if (!this->pending_.connect_state) return;
...
void WiFiComponent::process_roaming_scan_() {
  ...
  for (const auto& result : this->scan_result_) {
    ...
    if (result.ssid_ != current_ssid || result.get_bssid() == current_bssid) continue;
