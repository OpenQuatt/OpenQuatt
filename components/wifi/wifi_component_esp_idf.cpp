...
void WiFiComponent::wifi_lazy_init_() {
  if (this->wifi_initialized_) return;
  ...
  if (s_sta_netif == nullptr) s_sta_netif = esp_netif_create_default_wifi_sta();
...
bool WiFiComponent::wifi_mode_(optional<bool> sta, optional<bool> ap) {
  ...
  if (current_mode == set_mode) return true;
...
bool WiFiComponent::wifi_sta_connect_(const WiFiAP& ap) {
  // enable STA
  if (!this->wifi_mode_(true, {})) return false;
...
#else
      err =
          esp_wifi_sta_wpa2_ent_set_cert_key((uint8_t*)eap.client_cert, client_cert_len + 1, (uint8_t*)eap.client_key,
                                             client_key_len + 1, (uint8_t*)eap.password.c_str(), eap.password.length());
#endif
      if (err != ESP_OK) {
...
void WiFiComponent::wifi_process_event_(IDFWiFiEvent* data) {
  ...
  if (data->event_base == WIFI_EVENT && data->event_id == WIFI_EVENT_STA_START) {
    ...
    if (it.reason == WIFI_REASON_NO_AP_FOUND) {
      ESP_LOGW(TAG, "Disconnected ssid='%.*s' reason='Probe Request Unsuccessful'", it.ssid_len, (const char*)it.ssid);
    ...
    const bool needs_full = this->needs_full_scan_results_();
    {
      ...
      for (uint16_t i = 0; i < number; i++) {
        ...
        for (auto& res : this->scan_result_) {
          if (res.get_rssi() < weakest->get_rssi()) weakest = &res;
        }
