...
#else
template <typename T>
using wifi_scan_vector_t = FixedVector<T>;
#endif
...
class WiFiComponent final : public Component {
 public:
  ...
  bool requires_provisioning() {
    this->init_preferences_();
    return this->provisioning_required_;
  }
  ...
  bool has_sta_priority(const bssid_t& bssid) {
    for (auto& it : this->sta_priorities_) {
      if (it.bssid == bssid) return true;
    }
    return false;
  }
  int8_t get_sta_priority(const bssid_t bssid) {
    for (auto& it : this->sta_priorities_) {
      if (it.bssid == bssid) return it.priority;
    }
    return 0;
  }
 ...
 protected:
  ...
  bool all_networks_hidden_() const {
    if (this->sta_.empty()) return false;
    for (const auto& ap : this->sta_) {
      if (!ap.get_hidden()) return false;
    }
    return true;
  }
